...
## Phase 2 — hidden Diagnostics layer (how to access it)
The secret layer is now wired as a skeleton. To open it:

1. Go to the **Meditiraj** tab.
2. In the **"Prilagodi"** (custom minutes) field, type the secret code — default **`0`** —
   and tap the **min** button (or Done on the keyboard).
3. Instead of starting a session, the app switches to **"Privatnost i dozvole"**
   (the neutral Level-2 Diagnostics screen). To anyone watching it just looks like
   a custom duration that didn't start the timer.

**Leaving / panic exit:**
- Hardware **back** → returns to meditation.
- The **back arrow** in the top bar → returns to meditation.
- **Backgrounding the app** (Home, recents, screen off) → drops the hidden layer;
  re-opening always shows meditation, never where you left off.
- While on the hidden layer, **`FLAG_SECURE`** is on, so recents previews and
  screenshots show nothing.

**Change the code:** edit `AppViewModel.TRIGGER_CODE` (top of the companion object).
Keep it out of the real session range (e.g. `0`, or a value `>180`) so it never
collides with an actual meditation length. A user-chosen code in onboarding is a
Phase 3 item.

**Still a skeleton (Phase 3 fills these):** the Diagnostics screen is intentionally
empty/neutral — the real scan list, the Safety gate ("jesi na bezbednom mestu?"),
Leči/Seči, and SOS numbers are NOT here yet. That's by design (§3).

## Important guardrails (don't regress these in Phase 3)
