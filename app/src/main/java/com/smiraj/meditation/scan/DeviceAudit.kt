 ...
 *
 * Two sources:
 *  1. Real data from [SpecialAccessChecker.Result] — notification listeners
 *     and device admin apps become High/Medium findings immediately.
 *  2. Static guided items — biometrics/PIN, Bluetooth, SIM, browser sync,
 *     USB debugging, unknown-sources. These are always shown; the user
 *     reviews them manually.
 *
...
object DeviceAudit {
    ...
    fun buildCheckItems(specialAccess: SpecialAccessChecker.Result): List<DeviceCheckItem> {
        ...
        items += DeviceCheckItem(
            ...
            guidance = "Podešavanja → O telefonu → tapni 7 puta na broj verzije da otвориш meni. " +
